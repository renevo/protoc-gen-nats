...
import (
	...
	"github.com/renevo/protoc-gen-nats/internal/stringutils"
	"github.com/renevo/protoc-gen-nats/internal/typemap"
...
func (g *generator) generateServer(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) {
	...
	for _, method := range service.Method {
		...
		g.P(`	if err := group.AddEndpoint("`, endpointName, `", micro.HandlerFunc(func(req micro.Request) {`)
		...
		g.P(`		}`)
		g.P()
		g.P(`		`, outputVarName, `, err := impl.`, method.GetName(), `(rootCtx, &`, intpuVarName, `)`)
		g.P(`		if err != nil {`)
